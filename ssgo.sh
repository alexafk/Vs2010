...
check_sys(){
	if [[ -f /etc/redhat-release ]]; then
		...
		release="ubuntu"
	elif cat /proc/version | grep -q -E -i "centos|red hat|redhat"; then#!/usr/bin/env bash
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/local/bin:/usr/local/sbin:~/bin
export PATH
...
Set_port(){
	while true
		...
		read -e -p "(默认: 52102):" ss_port
		[[ -z "${ss_port}" ]] && ss_port="52102"
...
Set_password(){
	...
	read -e -p "(默认: captain):" ss_password
	[[ -z "${ss_password}" ]] && ss_password="captain"
	echo && echo "========================"
...
Set_cipher(){
 ...
 ${Tip} chacha20 系列加密方式无需额外安装 libsodium，Shadowsocks Go版默认集成 !" && echo
	read -e -p "(默认: 11. aead_aes_256_gcm):" ss_cipher
